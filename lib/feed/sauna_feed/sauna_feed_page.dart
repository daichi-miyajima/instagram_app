...
class SaunaFeedPage extends StatelessWidget {
  const SaunaFeedPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    Provider.of<SaunaFeedModel>(context, listen: false).fetchFirebaseData(); // fetchFirebaseData() を呼び出す
    return Scaffold(
      appBar: Header(),
      body: Consumer<SaunaFeedModel>(
        builder: (context, saunaFeedModel, child) {
          if (saunaFeedModel.feeds.isEmpty) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          } else {
            return ListView(
              children: saunaFeedModel.feeds
                  .map(
                    (feed) => ListTile(
                  title: Column(
                    children: [
