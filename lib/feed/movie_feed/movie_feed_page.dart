...
class MovieFeedPage extends StatelessWidget {
  const MovieFeedPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    Provider.of<MovieFeedModel>(context, listen: false).fetchFirebaseData(); // fetchFirebaseData() を呼び出す
    return Scaffold(
      appBar: Header(),
      body: Consumer<MovieFeedModel>(
        builder: (context, movieFeedModel, child) {
          if (movieFeedModel.feeds.isEmpty) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          } else {
            return ListView(
              children: movieFeedModel.feeds
                  .map(
                    (feed) => ListTile(
                  title: Column(
                    children: [
